...
import { ArrowLeft, ChevronRight, Send, MoreHorizontal } from "lucide-react";
...
import { ConnectionActionsSheet } from "@/components/app/connections/ConnectionActionsSheet";
        ...
        <div className="min-h-dvh pb-28 relative">
            ...
            >
                <div className="bg-white rounded-2xl shadow-[0_0_7px_rgba(0,0,0,0.15)] p-5">
                    ...
                    {profile.headline && (
                        <p className="text-[14px] font-sans text-[#715e55] mb-1">{profile.headline}</p>
                    )}
                    {profile.occupation && (
                        <p className="text-[13px] font-sans text-[#9b8479] mb-4">{profile.occupation}</p>
                    )}
                            ...
                            <div className="bg-[#fff8f0] border-l-4 border-[#d8602e] rounded-r-xl px-4 py-3 mb-3">
                                <p className="text-[13px] font-sans text-[#453933] leading-relaxed">
                                    ...
                                    <span className="font-semibold">
                                        {profile.common_traits.map((t) => formatDisplayText(t)).join(", ")}
                                    </span>{" "}
                    ...
                    {profile.prompts && profile.prompts.length > 0 && (
                        <div className="mb-5">
                            <h3 className="font-serif font-semibold text-[16px] text-[#181412] mb-2">Prompts</h3>
                    ...
                    {profile.bio && (
                        <div className="mb-5">
                            <h3 className="font-serif font-semibold text-[16px] text-[#181412] mb-2">About</h3>
                            <p className="text-[14px] font-sans text-[#453933] leading-relaxed">{profile.bio}</p>
                        </div>
                    )}
                    ...
                    {profile.hobbies && profile.hobbies.length > 0 && (
                        <div className="mb-5">
                            <h3 className="font-serif font-semibold text-[16px] text-[#181412] mb-2">Hobbies</h3>
                            <div className="flex flex-wrap gap-1.5">
                    ...
                    {profile.skills && profile.skills.length > 0 && (
                        <div className="mb-5">
                            <h3 className="font-serif font-semibold text-[16px] text-[#181412] mb-2">Skills</h3>
                            <div className="flex flex-wrap gap-1.5">
                    ...
                    {profile.intents && profile.intents.length > 0 && (
                        <div className="mb-5">
                            <h3 className="font-serif font-semibold text-[16px] text-[#181412] mb-2">Looking For</h3>
                            <div className="flex flex-wrap gap-1.5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {profile.offers.map((offer, i) => (
                                    <div key={i} className="bg-[#f8f7f6] rounded-xl p-3">
                                        ...
                                        <p className="text-[12px] font-sans text-[#715e55]">{offer.description}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="fixed bottom-16 md:bottom-0 left-0 right-0 md:left-56 bg-gradient-to-t from-white via-white to-white/0 pt-6 pb-5 px-5 z-20">
                <div className="max-w-lg mx-auto flex items-center gap-3">
                    <button
                        ...
                        className="flex-1 py-3.5 rounded-2xl bg-[#d8602e] text-white font-semibold font-sans text-[15px] flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-[0px_0px_10px_rgba(255,144,97,0.8)]"
                    >
                    ...
                    <button
                        ...
                        className="w-12 h-12 rounded-full border-2 border-[#e8e0da] bg-white flex items-center justify-center active:bg-[#f5ede6] transition-colors flex-shrink-0"
                        aria-label="More options"
                    >
                        <MoreHorizontal size={20} className="text-[#453933]" strokeWidth={1.5} />
                    </button>
                </div>
            </div>
